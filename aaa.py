...
#成功時の音楽
pyx.init(frequency = 44100, size = -16, channels = 2, buffer = 4096) # 初期設定
win_sounds = pyx.Sound("evening.mp3") # 再生ファイルを設定

canvas = tk.Canvas(root, height=HEIGHT, width=WIDTH, background='black')
canvas.pack()

# label_back = tk.Label(root, image=tkimg, bg="white")
...
def result():
    channel.stop()
    finish_loop = 1 # 再生回数(-1:無限ループ)
    win_sound = win_sounds.play(loops = finish_loop) # 音楽再生
